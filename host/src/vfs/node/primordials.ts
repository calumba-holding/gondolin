...
const primordials = {
  ArrayPrototypeIndexOf: (arr: unknown[], v: unknown) =>
    arr.indexOf(v as never),
  ...
  ArrayPrototypeSplice: (arr: unknown[], ...v: unknown[]) =>
    (Array.prototype.splice as (...args: any[]) => any).apply(
      arr as unknown[],
      v as unknown as any[],
    ),
  Boolean,
  ...
  Float64Array,
  FunctionPrototypeCall: (fn: Function, thisArg: unknown, ...args: unknown[]) =>
  ...
  StringPrototypeReplaceAll: (s: string, a: string, b: string) =>
    s.split(a).join(b),
